...
<div style="background:#CCCCCC;color:green;">权限管理 -》权限管理 </div>
<div class="orderlist">

<form action="<?php echo U('Rbac/addRoleHandle');?>" method="post">
<table border="1"  style="border-collapse:collapse;margin-top:20px;" width="1140" align="center">
	<legend style="margin-top:20px;"><center>权限管理</center></legend>
...
<tr style="background:#CCCCCC;">
 ...
 <?php if(is_array($nodeList)): foreach($nodeList as $key=>$row): ?><tr class="nodeList">
 		 <td><?php echo ($row["id"]); ?></td>
 		 <td class="node_chile"><p style="text-indent:<?php echo ($row['level']*25); ?>px">|-<?php echo ($row["title"]); ?></p></td>
 		 <td><?php echo ($row["name"]); ?></td>
 		 <td><?php echo ($row["sort"]); ?></td>
 		 <td>
 		 	<?php if($row['level'] == 1): ?>项目
 		 		<?php elseif($row['level'] == 2): ?>
                <span style="color:red;">模块</span>
 		 		<?php elseif($row['level'] == 3): ?>
                <span style="color:blue;">操作</span>
                <?php else: ?>
                <span style="color:#CCCCCC;">其他</span><?php endif; ?>
 		 	 
 		 </td>
 		 <td>
 		 	<!-- <a href="<?php echo U('Rbac/delNode',array('id'=>$row['id']));?>" style="text-decoration:none;">【 删除 】</a> -->
 		 	<a href="javascript:if(window.confirm('您确定要执行删除操作码？')){window.location='<?php echo U('Rbac/delNode',array('id'=>$row['id']));?>'}" style="text-decoration:none;">【 删除 】</a>
