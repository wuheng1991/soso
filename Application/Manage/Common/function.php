...
function getOrderCodeByID($id){
	if ($id == '') return '';
	if (!preg_match('/\d+/',$id)) return '';
	$info = M('Orders')->field('code')->where(' id = '.$id)->find();
	return $info['code'];
}
function createNum($key,$length){
...
function getEmails($id){
	if ($id == '') return '';
	$mem = M('Members');
	$info =  $mem->where('id = '.$id)->getField('username');
	return $info;
}
//得到用户是否发表信息 ——基因
function getStatus($id){
	if ($id == '') return '0';
	$note = M('Contents');
	$info = $note -> where("flag = 0 and id_order = ".$id)->order("id desc")->limit(1)->getField("status");
	return $info;
...
if($id == '') return '';
$model = M("pro_trace");
$file = $model -> where('id = '.$id) -> getField('file');
$arr = explode(".",$file);
$newFile = $arr[1];
return $newFile;
...
foreach($files as $row){
	 $file_path = $row['file'];
      ...
      $daye .= "<div id='fen_row_".$row['id']."' style=\"border:0px solid red;width:70px;height:75px;float:left;\">";

       if($exrt == 'jpg'){
      
      $daye .= "<a class=\"fen_row_a\" href='".$file_path."' data_value='".$row['id']."' target=\"_blank\" title=\"点击预览该图\"> <img src='".$file_path."' style=\"display:block;margin:5px 5px 0 5px;float:left;\" width=60 height=50/> </a>";
      }
      ...
      if($exrt == 'html'){
      $daye .= "<a class=\"fen_row_a\" href='".$file_path."' data_value='".$row['id']."' target=\"_blank\" title=\"点击预览或下载\"> ";
      $daye .="<img src=\"/Public/Picutue/html.png\" style=\"display:block;margin:5px 5px 0 5px;float:left;\" width=60 height=50/> </a>";
      }

      if($exrt == 'txt'){
      $daye .= "<a class=\"fen_row_a\" href='".$file_path."' data_value='".$row['id']."' target=\"_blank\" title=\"点击预览或下载\"> ";
      $daye .="<img src=\"/Public/Picutue/txt.png\" style=\"display:block;margin:5px 5px 0 5px;float:left;\" width=60 height=50/> </a>";
      }

       if($exrt == 'doc'){
      $daye .= "<a class=\"fen_row_a\" href='".$file_path."' data_value='".$row['id']."' target=\"_blank\" title=\"点击预览或下载\"> ";
      $daye .="<img src=\"/Public/Picutue/doc.png\" style=\"display:block;margin:5px 5px 0 5px;float:left;\" width=60 height=50/> </a>";
      }


       if($exrt == 'docx'){
      $daye .= "<a class=\"fen_row_a\" href='".$file_path."' data_value='".$row['id']."' target=\"_blank\" title=\"点击预览或下载\"> ";
      $daye .="<img src=\"/Public/Picutue/docx.png\" style=\"display:block;margin:5px 5px 0 5px;float:left;\" width=60 height=50/> </a>";
      }

       if($exrt == 'xls'){
      $daye .= "<a class=\"fen_row_a\" href='".$file_path."' data_value='".$row['id']."' target=\"_blank\" title=\"点击预览或下载\"> ";
      $daye .="<img src=\"/Public/Picutue/xls.png\" style=\"display:block;margin:5px 5px 0 5px;float:left;\" width=60 height=50/> </a>";
      }

      if($exrt == 'xlsx'){
      $daye .= "<a class=\"fen_row_a\" href='".$file_path."' data_value='".$row['id']."' target=\"_blank\" title=\"点击预览或下载\"> ";
      $daye .="<img src=\"/Public/Picutue/xlsx.png\" style=\"display:block;margin:5px 5px 0 5px;float:left;\" width=60 height=50/> </a>";
      }

       if($exrt == 'pdf'){
      $daye .= "<a class=\"fen_row_a\" href='".$file_path."' data_value='".$row['id']."' target=\"_blank\" title=\"点击预览或下载\"> ";
      $daye .="<img src=\"/Public/Picutue/pdf.png\" style=\"display:block;margin:5px 5px 0 5px;float:left;\" width=60 height=50/> </a>";
      }

       if($exrt == 'rar'){
      $daye .= "<a class=\"fen_row_a\" href='".$file_path."' data_value='".$row['id']."' target=\"_blank\" title=\"点击预览或下载\"> ";
      $daye .="<img src=\"/Public/Picutue/rar.png\" style=\"display:block;margin:5px 5px 0 5px;float:left;\" width=60 height=50/> </a>";
      }

       if($exrt == 'zip'){
      $daye .= "<a class=\"fen_row_a\" href='".$file_path."' data_value='".$row['id']."' target=\"_blank\" title=\"点击预览或下载\"> ";
      $daye .="<img src=\"/Public/Picutue/zip.png\" style=\"display:block;margin:5px 5px 0 5px;float:left;\" width=60 height=50/> </a>";
      }

 

  $daye .= "<div style=\"border:1px solid #CCCCCC;width:55px;height:25px;margin:50 auto;background:#cccccc;text-align:center;vertical-align:top;\">";
  $daye .= "<a class=\"fen_row_del\" style=\"border:0px solid red;height:25px;\" value='".$row['id']."' href=\"javascript:void(0)\"  style=\"color:red;\">删&nbsp;&nbsp;除</a>";
...
$check = $membermodel -> where('id ='.$id) -> getField("isCheck");
return $check;
}
...
$count = ceil($length/30);
$new_str="";
for($i=0;$i<=$count;$i ++){
$new_str = $new_str.substr($sequence,30 * $i,30)."<br/>";
}
return trim($new_str,"<br/>");
}
