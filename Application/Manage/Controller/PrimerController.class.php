...
class PrimerController extends CommonController{
	...
	$sequence = I("post.sequence",'','trim');  //序列
	$status = I("post.status",0,'intval'); //状态

	if($sequence != ""){
     $map['primername'] = array("like",'%'.$sequence.'%');
	}

	$map['status'] = $status; //状态

	$header = "A";
    // $date_new = substr(date("Y",time()),2,1);
    // echo $date_new."<br/>";
	// $boy = $header.$date_new;  //前半
    $boy = $header;
	// echo $boy;
    $map['primer_shui'] =array("like",$boy."%");
    ...
    $sequence_info = $orders_primers -> where($map) -> order("id asc") -> page($page)->limit($pageNum)-> select();
	$countNum = $orders_primers->where($map)->count();
	$pages = ceil($countNum / $pageNum);

	$this -> assign("sequence_info",$sequence_info);
