...
class CexuController extends CommonController{
    public function index(){
		if (UserController::isLogin()){
			
			$this->display('Cexu/index');
		} else {
	...
	public function main(){
		...
		$sequencingList = $order->where(' ordertype = 1 and status = 2')->order('addtime desc')->limit($pageNum)->select();
		...
		$primersList = $order->where(' ordertype = 2 and status = 2')->order('addtime desc')->limit($pageNum)->select();
  ...
  //        $this->assign('page_b',$show_b);// 赋值分页输出
		...
		// $orderList = $order->where(' ordertype = 3')->order('addtime desc')->page($page_c)->limit($pageNum)->select();
		 // var_dump($orderList);
		$orderList = $order->where(' ordertype = 3 and status = 2')->order('addtime desc')->limit($pageNum)->select();
	...
	public function sequencinginfo(){
		...
		$this->display('Cexu/sequencinginfo');
	}
				...
				if ($orderInfo && $orderInfo['status'] == 2){
					if ($sequencinginfo['number'] == ''){
						...
						if ($orders_sequencing->data($data)->save()){
							$this->success('生成成功',U('Cexu/sequencingShow','id='.$sequencinginfo['orderid']));
						}
					} else {
						$this->error('已生成！');
					}
		...
		if ($id > 0){
			...
			if ($orderInfo && $orderInfo['status'] == 2){
				...
				if (!empty($sequencingList)){
					...
					$this->success('生成成功',U('Cexu/sequencingShow','id='.$id));
				} else {
		...
		if ($id > 0){
			...
			if ($orderInfo && $orderInfo['status'] == 2){
				...
				if ($list){
					...
					$this->success('更新结果成功',U('Cexu/sequencingShow','id='.$id));
				} else{
					$this->error('参数错误！');
				}
			} else $this->error('订单未确认，请先确认订单！');
		...
		if ($id > 0){
			...
			if ($orderInfo){
				...
				if ($order->data($data)->save()){
					$this->success('确认订单成功！',U('Cexu/sequencingShow','id='.$id));
